...
df_dummies['Date']=pd.to_datetime(df.Date)
...
# then drop Date from X.
X = X.drop(['Date'], axis=1)
X

y = df_dummies[['Weekly_Sales', 'Date']]
y = y.sort_values('Date')
y = y.drop(['Date'], axis=1)
y
# use the earlier year to train and test with later years instead of using train test split.
# X_train, X_test, y_train, y_test = train_test_split(X, y)
split_boundary = round(np.shape(X_train)[0]*0.75)
X_train = X.iloc[0:split_boundary, :]
X_train
y_train = y.iloc[0:split_boundary, :]
y_train
X_test = X.iloc[split_boundary:, :]
X_test
y_test = y.iloc[split_boundary:, :]
y_test
